...
module.exports = function (RED) {
    ...
    RED.nodes.registerType("questdb", QuestDbConfigNode/* , {
        credentials: {
            username: { type: "text" },
    ...
    async function writePoints (msg, node, done) {
        try {
            ...
            const unit = Object.prototype.hasOwnProperty.call(msg, 'precision') ? msg.precision : node.precision;
            if (!table) {
    ...
    function QuestDBBatchNode (n) {
        ...
        node.on("input", async function (msg, send, done) {
            if (node.questdbConfig.protocol === 'tcp' && node.questdbConfig.protocol === 'tcps') {
                await node.questdbConfig.sender.connect();
            }

            for (const element of msg.payload) {
                const sender = node.questdbConfig.sender(element.table);
